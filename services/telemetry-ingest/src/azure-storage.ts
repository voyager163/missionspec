...
import type { Storage, TelemetryRecord } from './contract.js';
...
export function createStorageAdapter(client: UploadClient, ruleId: string): Storage {
  return {
...
export async function createAzureStorage(config: AzureConfig): Promise<Storage> {
  ...
  return createStorageAdapter(client, config.ruleId);
}
